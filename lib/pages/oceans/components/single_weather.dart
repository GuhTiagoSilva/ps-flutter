...
import 'package:ocean_tech/services/ocean_max_temperature_service.dart';
...
class SingleWeather extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var oceanService =
        OceanMaxTemperatureService().getMaxTemperaturaOfAnOcean();
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Temperatura Máxima dos Oceanos',
                ...
                FutureBuilder<List<OceanMaxTemperatureModel>>(
                  future: oceanService,
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.done) {
                      if (snapshot.hasData) {
                        return buildData(
                            snapshot.data as List<OceanMaxTemperatureModel>, 2);
                      } else {
                        return Center(
                        ...
                        if (snapshot.connectionState == ConnectionState.done) {
                          if (snapshot.hasData) {
                            return buildOceanData(
                                snapshot.data as List<OceanMaxTemperatureModel>,
                                0);
                          } else {
                            return Center(
                              child: Text("Nenhum oceano cadastrado!"),
  ...
  Column buildData(List<OceanMaxTemperatureModel> oceans, int index) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
          oceans[index].maxTemperature.toString() + "°C",
          style: GoogleFonts.lato(
            fontSize: 85,
  ...
  Column buildOceanData(List<OceanMaxTemperatureModel> oceans, int index) {
    return Column(
      children: [
        Text(
